...
RegressionStrategyInstance::RegressionStrategyInstance() {
}

RegressionStrategy::RegressionStrategy(utils::Verbosity verbosity)
...
static class RegressionStrategyCategoryPlugin
    : public plugins::TypedCategoryPlugin<RegressionStrategy> {
public:
    RegressionStrategyCategoryPlugin()
        : TypedCategoryPlugin("RegressionStrategy") {
        document_synopsis(
            "This page describes the various regression strategies for "
            "Cartesian sets with derived variables supported "
            "by the planner.");
    }
} _category_plugin;
